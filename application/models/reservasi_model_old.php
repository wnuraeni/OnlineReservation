...
class reservasi_model_old extends CI_Model{
    function __contruct(){
        parent :: __construct();
    }
...
function pesan($data){
    ...
    $data_booking = array('id_member'=>  $id_member,'id_lapangan'=>$data['id_lapangan'],
...
function get_detail_reservasi($id_reservasi=null){
    ...
    $html .= '<table>
            ...
            </table>'.(($data_reservasi[0]->status_pembayaran != "lunas")?
            '<button class="button" onclick="pembayaran('.$id_reservasi.')" style="font-weight:bold; color: black">Tambah Pembayaran</button>
            ':'<strong>Catatan Pembayaran</strong>').'
            <table><tr><th>Tanggal Pembayaran</th><th>Jumlah Bayar</th><th>Bukti Pembayaran</th><th>Keterangan</th><th>Status Pembayaran</th><th>Aksi</th></tr>';
    ...
    $html .= '</table>
            <button onclick="window.location.href=\''.base_url().'index.php/kasir/reservasi_controller/claim_refund/'.$id_reservasi.'\'" style="font-weight:bold; color: black">Claim Refund</button>
...
function template_konfirmasi($id_reservasi){
      $reservasi = $this->ambil_reservasi(array('id_booking'=>$id_reservasi));
         $html = '<h5>Terima Kasih Telah Melakukan Pesanan</h5>
                ...
                <tr><td>Lama Pemakaian</td><td>'. $reservasi[0]->lama_pemakaian.'</td></tr>
